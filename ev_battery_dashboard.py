...
#         # Filter the dataset down to just the chosen battery
#         df = master_df[master_df['Battery_ID'] == selected_bat].reset_index(drop=True)
#         st.success(f"Currently tracking: {selected_bat}")
#     else:
#         st.error("⚠️ The uploaded file does not contain a 'Battery_ID' column.")
# else:
#     st.info("💡 Please upload the 'demo_6_batteries_telemetry.csv' file to begin.")
...
# if df is not None:
#     if 'Cycle' not in df.columns or 'Capacity' not in df.columns:
#         st.error("⚠️ Active data must contain 'Cycle' and 'Capacity' columns.")
#     else:
#         # 1. Pipeline Calculus
#         initial_capacity = df['Capacity'].iloc[0]
#         current_capacity = df['Capacity'].iloc[-1]
